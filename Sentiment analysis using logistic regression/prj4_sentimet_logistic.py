...
with open(r"D:\project work\Projects\data\full_set.txt") as f:
    content = f.readlines()

#Remove leading and trailing white space
content = [x.strip() for x in content]
print("Content after Stripping white spaces: ", content)
...
#"full_remove" takes a string x and a list of characters "removal_list" and
# returns with all the characters in removal_list replaced by ' ' .
...
stop_set = set(['the', 'a', 'an', 'i', 'he', 'she', 'they', 'to', 'of', 'it', 'from', 'one', 'two', 'ft'])
# Remove stop words
sents_split = [x.split() for x in sents_lower]
sents_processed = [" ".join(list(filter(lambda a: a not in stop_set, x))) for x in sents_split]

#Let us look at the sentences:
print("Final Sentences to Analyze: ",sents_processed[0:10])
